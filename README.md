...
dllm 開源不是為了取代誰，而是希望讓 GB-10 這類設備的生態更完整。目標方向：

| 方向 | 說明 |
|------|------|
| **⚡ 優化效能** | 更快的推理速度、更低的記憶體 footprint、更好的 vLLM 整合 |
| **🔌 更高兼容** | 支援更多模型格式（HF / GGUF / MLX）、更多硬體平台 |
| **🛠️ 完善 CLI** | `dllm run` 更順暢、`dllm serve` 更穩定、指令更直覺 |
| **📋 管理功能** | 模型管理、Token 計算、安全審計、安裝包 |
| **🌐 社群共創** | 你的使用場景可能跟我不一樣，你的需求就是產品進化的方向 |

無論是程式碼、文件、想法、還是 Bug 回報，都對這個專案有貢獻。

## 貢獻方式

| 方式 | 說明 |
|------|------|
| **使用回報** | 在哪台設備上跑？遇到什麼問題？開個 Issue 告訴我 |
| **程式碼** | Rust、vLLM 整合、模型管理策略、CLI 功能 |
| **測試** | 在不同的 GB-10 設備（ASUS / Dell / 銘凡）上幫忙驗證 |
| **建議** | CLI 怎麼設計更好？管理功能缺什麼？你的意見就是方向 |
